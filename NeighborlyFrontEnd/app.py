...
def delete_ad_request(id):
    response = requests.delete(settings.API_URL + '/deleteAdvertisement?code=d4VABNMIzhiPPgHECrioIqdefNNUiTK6Fac7onx5Wbuysmao0DUafw==&id=' + id)
    if response.status_code == 200:
        return redirect(url_for('home'))

@app.route('/post/delete/<id>', methods=['POST'])
def delete_post_request(id):
    response = requests.delete(settings.API_URL + '/deletePost?code=KfxwLgXYufFnau7XXojVD4oerY/zqdRo9VtC1nrE7BU32OAuBy9D5A==&id=' + id)
    if response.status_code == 200:
        return redirect(url_for('home'))
